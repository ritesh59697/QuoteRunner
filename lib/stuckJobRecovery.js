 ...
 *
 * Only the "ai-dispatch/... command failed ... session=job:<FULL_ID>:my:"
 * line shape carries the untruncated jobId. The "AI session done" summary
 * line ellipsis-truncates it for readability (0xd30a22…1eda39), which is not
 * enough to call `onchainos agent status` or `apply` with — those need the
 * full id.
 */
function findFailedDispatchJobIds(logText) {
  ...
  for (const line of lines) {
    ...
    const m = line.match(/session=job:(0x[0-9a-fA-F]+):my:/);
    if (m) ids.add(m[1]);
  }
  return [...ids];
}
