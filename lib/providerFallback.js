...
const A2A_BIN = process.env.OKX_A2A_BIN || '/Users/ritesh/.hermes/node/bin/okx-a2a';
...
const PROVIDER_DEAD_SIGNATURES = [
  ...
  // 2026-07-17: headless claude/hermes began returning HTTP 400 "Third-party
  // apps now draw from your extra usage, not your plan limits. Add more at
  // claude.ai/settings/usage". A billing wall, but from the daemon's side it is
  // a dead provider — the CLI exits 1 and the job is dropped. It matched nothing
  // above, so the watchdog stayed bound to a broken provider while claude worked.
  /extra usage/i,
  /third-party apps/i,
  /settings\/usage/i,
];
...
function probeCommand(provider) {
  switch (provider) {
    ...
    case 'codex':
      return { bin: 'sh', args: ['-c', 'codex exec --ephemeral "reply with: OK" < /dev/null'] };
    case 'hermes':
