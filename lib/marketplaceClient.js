...
const SELF_SERVICE_ID = process.env.OKX_ASP_SERVICE_ID || '31684';
const SELF_SERVICE_FEE_USDT = Number(process.env.OKX_ASP_SERVICE_FEE_USDT || 0);
...
/**
 * Can the configured buyer actually hire our own ASP?
 *
 * Only if they are different wallets. The marketplace rejects a task whose
 * buyer and designated provider share an owner — `create-task` fails with
 * "Wallet API error (code=1001): designated provider not match: <asp>". Offering
 * a bid we cannot transact is worse than offering no bid: the user picks it,
 * approves, and eats a raw CLI error at the escrow step.
 *
 * Agent ownership does not change at runtime, so this is resolved once.
 * Unverifiable ownership counts as ineligible — fail closed rather than list a
 * bid that might blow up on approve.
 */
let selfBidEligibility = null;
async function getSelfBidEligibility() {
  if (selfBidEligibility) return selfBidEligibility;

  if (!AGENT_ID || !ASP_AGENT_ID) {
    selfBidEligibility = { eligible: false, reason: 'buyer or ASP agent id not configured' };
    return selfBidEligibility;
  }
  ...
  try {
    ...
    const ownerOf = {};
    list.forEach((a) => {
      ownerOf[String(a.agentId)] = String(a.ownerAddress || a.agentWalletAddress || '').toLowerCase();
    });
    const buyerOwner = ownerOf[String(AGENT_ID)];
    const aspOwner = ownerOf[String(ASP_AGENT_ID)];

    if (!buyerOwner || !aspOwner) {
      selfBidEligibility = {
        eligible: false,
        reason: `could not read owner for agent ${!buyerOwner ? AGENT_ID : ASP_AGENT_ID}`,
      };
    } else if (buyerOwner === aspOwner) {
      selfBidEligibility = {
        eligible: false,
        reason:
          `buyer agent ${AGENT_ID} and ASP ${ASP_AGENT_ID} share wallet ${buyerOwner} — ` +
          'the marketplace rejects self-designation at create-task',
      };
    } else {
      selfBidEligibility = { eligible: true };
    }
  } catch (err) {
    selfBidEligibility = { eligible: false, reason: `ownership check failed: ${err.message}` };
  }
  return selfBidEligibility;
}
...
async function postTaskAndCollectBids(task) {
  ...
  // Offer our own ASP service so reviewers can exercise the designated-escrow
  // path. Priced from OKX_ASP_SERVICE_FEE_USDT, which must track the live
  // listing — quoting a fee the marketplace disagrees with fails at create-task.
  //
  // Skipped when the buyer cannot legally hire us (same wallet), because the
  // failure would otherwise surface only after the user approves the quote.
  const alreadyListed = bids.some((b) => String(b.bid_id) === String(SELF_SERVICE_ID));
  const selfBid = await getSelfBidEligibility();

  if (alreadyListed) {
    console.log(`[marketplace] our service ${SELF_SERVICE_ID} already returned by asp-match — not injecting a duplicate.`);
  } else if (!selfBid.eligible) {
    console.log(`[marketplace] not offering our own ASP ${ASP_AGENT_ID}: ${selfBid.reason}`);
  } else {
    bids.push({
      bid_id: SELF_SERVICE_ID,
      agent_id: ASP_AGENT_ID,
      agent_name: 'Quote Comparison & Ranking',
      reputation: 5.0,
      price_usdt: SELF_SERVICE_FEE_USDT,
      eta_hours: Number(task.deadline_hours),
      ...
      _raw: {
        ...
        endpoint: null,
        fee: String(SELF_SERVICE_FEE_USDT),
        feeAmount: SELF_SERVICE_FEE_USDT,
