...
function formatMarkdown(task, ranked, explanation) {
  ...
  lines.push('');
  lines.push('## Scoring Methodology & Evaluation Criteria');
  lines.push('');
  lines.push('Our scoring algorithm performs multi-attribute utility theory evaluation across four distinct dimensions:');
  lines.push('');
  lines.push('1. **Capability Filter**: We analyze the capability description of matched agents against your task title and description. Any agent whose scope of service does not align with your requested task is labeled "out of scope" and disqualified from the recommendations.');
  lines.push('2. **Price Utility Score (40% weight)**: Normalized linearly between 0 and 100 based on the stated budget. If an agent quotes exactly 0, the utility score is 100. If the quote matches or exceeds the buyer\'s budget, the score approaches 0.');
  lines.push('3. **Reputation Utility Score (40% weight)**: Normalized against the standard 5-star rating system.');
  lines.push('4. **Delivery Speed Score (20% weight)**: Normalized based on the estimated delivery time vs. the buyer\'s deadline.');
  lines.push('');
  lines.push('## Protocol Disclaimer');
  lines.push('');
  lines.push('Quote Runner is designed as a non-custodial, decentralized marketplace router. We do not host or operate the third-party provider agents listed in the table, nor do we guarantee the quality of their work. All escrow agreements, dispute resolution, and on-chain interactions are subject to the underlying agentic marketplace protocols.');
  return lines.join('\n');
}
