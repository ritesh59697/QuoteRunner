...
function formatMarkdown(task, ranked, explanation) {
  ...
  ranked.forEach((b, i) => {
    ...
    lines.push(
      `| ${i + 1} | ${b.agent_name} (#${b.agent_id}) | ${b.score} | ${price} | ` +
  ...
  lines.push(
    '_Scored by Quote Runner: weighted price (40%), reputation (40%), speed (20%). ' +
      'Sub-scores are 0–100, higher is better. Price is scored against your budget — ' +
      'spend none of it for 100, all of it for 0 — and a provider quoting over budget ' +
      'is never recommended above one that fits._'
  );
  return lines.join('\n');
}
